"""@file tt_prod_ML4EFT.py Module containing the analytical cross-sections for 
top-quark pair production at the parton level in the SMEFT"""
...
class crossSectionSMEFT:
    ...
    def sigma_part_gg(self, hats, ctGRe, cut, order):
        ...
        kappa_11 = ((v**2 * yt**2 * asQCD) / (24 * LambdaSMEFT**4 * hats**3)) * (
            6 * np.sqrt(hats**5 * (hats - 4 * mt**2))
            + hats
            * mt**2
            * (
                -3 * np.sqrt(hats * (hats - 4 * mt**2))
                - 8 * hats * np.log(1 - sqrt)
                + 8 * hats * np.log(sqrt + 1)
            )
        )
        kappa_1 = (
            (np.sqrt(np.pi) * v * yt * mt * asQCD)
            / (6 * LambdaSMEFT**2 * hats**2 * np.sqrt(2))
            * (
                9 * np.sqrt(hats * asQCD * (hats - 4 * mt**2))
                + 8
                * hats
                * np.sqrt(asQCD)
                * (
                    np.log(1 - np.sqrt(1 - 4 * mt**2 / hats))
                    - np.log(np.sqrt(1 - 4 * mt**2 / hats) + 1)
                )
            )
        )
        sm = (
            ...
            / (12 * hats**3)
            * (
                4 * mt**4 * (np.log(1 - sqrt) - np.log(sqrt + 1))
                + mt**2
                * (
                    31 * np.sqrt(hats * (hats - 4 * mt**2))
                    + 16 * hats * np.log(1 - sqrt)
                    - 16 * hats * np.log(sqrt + 1)
                )
                + hats
                * (
                    7 * np.sqrt(hats * (hats - 4 * mt**2))
                    + 4 * hats * np.log(1 - sqrt)
                    - 4 * hats * np.log(sqrt + 1)
                )
            )
        )
    ...
    def sigma_part_qq(self, hats, cuGRe, ctu8, order):
        ...
        kappa_11 = (
            sqrt
            * (8 * np.pi * v**2 * yt**2 * asQCD * (8 * mt**2 + hats))
        ...
        kappa_22 = (
            (2.0 / 9.0)
            * (hats * sqrt - mt**2 * sqrt)
            / (12 * 4 * np.pi * LambdaSMEFT**4)
        )

        kappa_1 = -(8 * np.sqrt(2 * np.pi) * v * mt * asQCD ** (3 / 2) * sqrt) / (
            9 * hats * LambdaSMEFT**2
        )
    ...
    def dsigma_part_qq_dpt(self, hats, pt, ctGRe, cut, order):
        ...
        dsigma_dpT_1 = (
            2 * np.pi * np.tan(theta) * me_sq_theta / (64 * np.pi**2 * S * pi)
        )
        dsigma_dpT_2 = (
            2
            * np.pi
...
def weight(sqrts, mu, x1, x2, c, order):
    ...
    w_e = (xsec.sigma_part_gg(hats, ctGRe, cut, order)) * (
        p.xfxQ(21, x1, mu) * p.xfxQ(21, x2, mu)
    )
...
def weight_pt(sqrts, pt, mu, x1, x2, c, order):
    ...
    pdfs_up = np.sum(
        [
            p.xfxQ(pid, x1, mu) * p.xfxQ(-pid, x2, mu)
            + p.xfxQ(-pid, x1, mu) * p.xfxQ(pid, x2, mu)
            for pid in flavor_up
        ]
    )
    pdfs_down = np.sum(
        [
            p.xfxQ(pid, x1, mu) * p.xfxQ(-pid, x2, mu)
            + p.xfxQ(-pid, x1, mu) * p.xfxQ(pid, x2, mu)
            for pid in flavor_down
        ]
    )

    w_e = xsec.dsigma_part_gg_dpt(hats, pt, ctGRe, cut, order) * (
        p.xfxQ(21, x1, mu) * p.xfxQ(21, x2, mu)
    )
...
def dsigma_dmtt_dy(y, mtt, c=None, order=None):
    ...
    if c is None:
        c = np.zeros(2)
    if mtt == 2 * mt:
        return 0  # if at threshold return zero
    if np.abs(y) < np.log(
        np.sqrt(s) / mtt
    ):  # check whether x = {mtt, y} falls inside the physically allowed region
        x1 = mtt / np.sqrt(s) * np.exp(y)
        x2 = mtt / np.sqrt(s) * np.exp(-y)
        dsigma_dmtt_dy = (
            2 * mtt / s * v_weight(mtt, 91.188, x1, x2, c, order) / (x1 * x2)
        )
...
def dsigma_dmtt(mtt, c=None, order=None):
    ...
    dsigma_dmtt = integrate.fixed_quad(
        dsigma_dmtt_dy_vec, y_min, y_max, args=(mtt, c, order), n=100
    )[0]
    return dsigma_dmtt
...
if __name__ == "__main__":
    ...
    Mtt = 1
    ytt = 2
    # Cross section in pb/TeV -> pb / GeV
    print(f"Cross section: {dsigma_dmtt_dy(ytt, Mtt) * 1e-3}")
